...
const MISSIONS_DATA = {
  iniciante: {
    ...
    fase1_summary: {
      done: "Operador, preste muita atenção. 90% das pessoas que tentam vender na internet falham exatamente aqui. Elas tentam vender 'tudo para todos'. Nesta fase, nós cortamos o ruído. Definimos exatamente QUEM vamos atacar e COMO vamos nos posicionar.",
      result: "Você agora possui uma Bússola Estratégica. Enquanto os amadores estão atirando no escuro, você tem um alvo laser."
    },
    fase2: [
    ...
    fase2_summary: {
      done: "Ideias não pagam boletos. Nesta fase, transformamos o conceito abstrato em um Ativo Digital tangível. O deploy da página de vendas é o momento em que sua loja abre as portas para o mundo.",
      result: "Sua Infraestrutura de Guerra está montada. Você tem um produto real, um link de pagamento funcional e uma página persuasiva."
    },
    fase3: [ 
    ...
    fase3_summary: {
      done: "Chegou a hora da verdade: O Combate. Configuramos armadilhas de lucro (Order Bump) para aumentar o ticket médio sem esforço extra. A estratégia X1 no WhatsApp é trabalhosa, mas essencial para validação.",
      result: "Prova de Conceito (POC). O objetivo desta fase é fazer o celular vibrar com a notificação de venda."
    }
  },
  mestre: {
    fase1: [
    ...
    fase1_summary: {
      done: "Bem-vindo à Liga Profissional. Acabou a dependência da sorte do algoritmo orgânico. Aqui nós compramos dados. Configuramos o Business Manager e o Pixel para rastrear cada passo do cliente.",
      result: "Controle de Tráfego. Você agora tem um painel de controle. Começamos a transformar publicidade em matemática previsível."
    },
    fase2: [
    ...
    fase2_summary: {
      done: "Nesta fase, focamos em Eficiência e LTV. O cliente mais caro é o novo; o mais barato é aquele que já comprou. Com Upsells e Downsells, aumentamos o lucro por venda instantaneamente.",
      result: "Máquina de Lucro Otimizada. Você está construindo um ativo que vale dinheiro a longo prazo."
    },
    fase3: [
    ...
    fase3_summary: {
      done: "O estágio final da metamorfose: de Vendedor para Empresário. A escala exige coragem para investir pesado e inteligência para não quebrar.",
      result: "Liberdade Real. Você não tem mais um emprego que você mesmo criou; você tem um negócio."
    }
  }
};
  ...
  <div className="my-12 relative overflow-hidden rounded-2xl border border-cyan-500/30 bg-gradient-to-r from-slate-900 via-cyan-950/20 to-slate-900 shadow-2xl">
    ...
    <div className="p-8 md:p-10 relative z-10 flex flex-col md:flex-row items-center gap-8 justify-between">
      <div className="flex-1">
        <div className="inline-block px-3 py-1 bg-cyan-500/20 text-cyan-400 text-xs font-black uppercase tracking-widest rounded mb-4">
      ...
      <a 
        href="https://nexusdigital.net.br" 
        target="_blank"
        className="shrink-0 bg-cyan-500 hover:bg-white text-black font-black text-lg px-8 py-4 rounded-xl shadow-[0_0_20px_rgba(6,182,212,0.4)] transition-all transform hover:scale-105 flex items-center gap-3"
      >
...
const NexusLogo = ({ className = "h-12", showText = true }) => {
    ...
    <div className={`flex items-center justify-center ${className}`}>
      {!error ? (
        <img 
          src="/logo.jpg" 
          alt="Nexus Digital" 
          className="h-full w-auto rounded-full shadow-lg border-2 border-white/10 hover:border-cyan-500/50 transition-all cursor-pointer object-contain"
          onError={() => setError(true)}
        />
      ) : (
        <div className="flex flex-col justify-center items-center bg-cyan-950/30 p-2 rounded-lg border border-cyan-500/30">
            <BrainCircuit className="text-cyan-400 mb-1" size={24}/>
      ...
      {(!error && showText) && (
          <div className="hidden md:flex flex-col justify-center ml-3">
            ...
            <span className="text-[9px] font-bold text-cyan-400 tracking-[0.4em] leading-none mt-1 drop-shadow-md uppercase">Digital</span>
          </div>
      )}
    </div>
  );
...
const Button = ({ children, onClick, variant = 'primary', className = '' }) => {
  ...
  const variants = {
    primary: "bg-cyan-600 hover:bg-cyan-500 text-white shadow-cyan-900/20 hover:scale-[1.02]",
...
const GlassCard = ({ children, className = "", hoverEffect = false, onClick }) => (
  <div 
    ...
    className={`
      ...
      ${hoverEffect ? 'hover:bg-slate-800/60 hover:border-cyan-500/30 hover:shadow-[0_0_30px_-10px_rgba(6,182,212,0.3)] transition-all duration-500 group cursor-pointer' : ''}
      ${className}
    `}
  >
    {children}
...
const ToolCard = ({ tool }) => (
  <a href={tool.url} target="_blank" rel="noopener noreferrer" className="block h-full group">
    <GlassCard hoverEffect={true} className="flex flex-col h-full cursor-pointer p-6">
      <div className="flex justify-between items-start mb-4">
        <h4 className="font-bold text-white text-xl tracking-tight group-hover:text-cyan-400 transition-colors flex items-center gap-2">
          {tool.name}
        </h4>
        <span className={`text-[11px] uppercase tracking-widest bg-white/5 px-3 py-1 rounded border border-white/10 font-bold transition-colors ${tool.color ? tool.color : 'text-slate-300'} group-hover:border-cyan-500/30`}>
          {tool.tag}
        </span>
      </div>
      ...
      <div className="mt-auto w-full py-4 rounded-lg bg-white/5 border border-white/10 text-slate-300 text-base font-bold flex items-center justify-center gap-2 group-hover:bg-cyan-500 group-hover:text-black group-hover:border-cyan-500 transition-all duration-300">
...
const TaskItem = ({ id, text, xp, done, onToggle }) => (
  <div onClick={() => onToggle(id, !done)} className={`relative p-6 border-b border-white/5 last:border-0 flex items-center gap-6 cursor-pointer group overflow-hidden transition-all duration-300 select-none ${done ? 'bg-cyan-950/10' : 'hover:bg-white/[0.02]'}`}>
    <div className={`absolute left-0 top-0 bottom-0 w-1.5 transition-all duration-300 ${done ? 'bg-cyan-500' : 'bg-transparent group-hover:bg-slate-700'}`} />
    <div className={`w-8 h-8 rounded-lg border-2 flex items-center justify-center transition-all duration-300 shrink-0 ${done ? 'bg-cyan-500 border-cyan-500 scale-110' : 'border-slate-600 group-hover:border-cyan-400'}`}>
    ...
    <div className={`px-4 py-1.5 rounded-full text-sm font-bold border transition-all duration-300 ${done ? 'bg-cyan-500/20 text-cyan-400 border-cyan-500/30' : 'bg-white/5 text-slate-500 border-white/5 group-hover:border-white/10'}`}>
      +{xp} XP
    </div>
  </div>
);
...
const PhaseSummary = ({ data, setActiveTab }) => {
  ...
  return (
    <div className="mt-6 mb-4 mx-6">
      <button onClick={() => setIsOpen(!isOpen)} className="w-full flex items-center justify-between p-6 bg-gradient-to-r from-slate-900 to-slate-800 rounded-xl border border-white/10 hover:border-cyan-500/30 transition-all group shadow-lg">
        <div className="flex items-center gap-4">
          <div className="p-3 bg-cyan-500/10 rounded-lg text-cyan-400 group-hover:text-cyan-300"><Target size={28} /></div>
        ...
        <div className="mt-3 bg-slate-900/80 rounded-xl border border-white/5 overflow-hidden animate-fadeIn">
          <div className="p-8 grid gap-8 md:grid-cols-1">
            ...
            <div className="space-y-4 pt-6 border-t border-white/5">
              <h4 className="text-lg uppercase tracking-widest text-blue-400 font-black flex items-center gap-3"><Trophy size={24} /> Resultado Esperado</h4>
              <p className="text-slate-300 text-lg leading-relaxed border-l-4 border-blue-500/30 pl-6 py-1">{data.result}</p>
            </div>
            ...
            <div className="mt-8 pt-6 border-t border-white/10 text-sm text-slate-500 flex items-start gap-3 opacity-80 hover:opacity-100 transition-opacity">
               ...
               <p className="text-base">
                 ...
                 Para suporte sobre o método Nexus, acesse a aba <button onClick={() => setActiveTab('contacts')} className="text-cyan-400 hover:underline font-bold">Contato</button>.
               </p>
            </div>
          </div>
        </div>
...
const LevelUpModal = ({ isOpen, onClose, onConfirm, currentLevel }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/95 z-[100] flex items-center justify-center p-4 backdrop-blur-md animate-fadeIn">
      <GlassCard className="max-w-lg w-full border-cyan-500/30 p-10 shadow-2xl shadow-cyan-900/40">
        <div className="text-center mb-10">
          <Award size={80} className={`mx-auto mb-6 drop-shadow-[0_0_35px_rgba(6,182,212,0.6)] ${isUpgrade ? 'text-cyan-400' : 'text-purple-400'}`} />
        ...
        <div className="flex flex-col gap-4 mt-8">
          <Button onClick={() => onConfirm(targetLevel)} variant="primary" className="py-5 text-xl font-black tracking-wide w-full">
...
const Dashboard = ({ niche, completedTasks = [], onTaskToggle, xp, level, resetApp, setLevel }) => {
  ...
  const NavButton = ({ id, icon: Icon, label, onClick }) => (
    <button 
      ...
      className={`w-full flex items-center gap-5 px-6 py-5 rounded-2xl font-bold text-lg transition-all ${activeTab === id ? 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/20' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
    >
      <Icon size={26} />
      <span>{label}</span>
    </button>
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex text-slate-200 font-sans selection:bg-cyan-500/30">
      <GlobalStyles />
      <StickyBuyButton /> {/* BOTÃO FLUTUANTE SEMPRE ATIVO */}
      ...
      <div className="md:hidden fixed top-0 w-full z-50 bg-slate-950/90 backdrop-blur border-b border-white/5 px-6 h-24 flex items-center justify-between shadow-xl">
        <NexusLogo showText={false} className="h-16" />
      ...
      <main className="flex-1 md:ml-80 p-6 md:p-16 pt-32 md:pt-16 max-w-[1600px] mx-auto w-full">
        ...
        <div className="flex flex-col md:flex-row gap-10 items-start md:items-center justify-between mb-16">
          ...
          <div className="flex items-center gap-8 bg-slate-900 border border-white/5 p-4 pr-8 rounded-3xl shadow-2xl">
             <div className={`h-20 w-20 rounded-2xl flex items-center justify-center shadow-lg ${level === 'mestre' ? 'bg-gradient-to-br from-purple-600 to-indigo-600 shadow-purple-900/30' : 'bg-gradient-to-br from-cyan-600 to-blue-600 shadow-cyan-900/30'}`}>
               <Crown size={36} className="text-white" />
             </div>
             <div className="text-left">
               <div className="text-xs font-black text-slate-500 uppercase tracking-wider mb-1">Patente Atual</div>
               <div className={`font-black text-3xl uppercase tracking-tight ${level === 'mestre' ? 'text-purple-400' : 'text-cyan-400'}`}>{level}</div>
        ...
        {activeTab === 'dashboard' && (
            <div className="animate-fadeIn space-y-12">
                        ...
                        <div className="bg-slate-950 rounded-[28px] p-8 md:p-10 flex flex-col md:flex-row items-center justify-between gap-8">
                            <div>
                                ...
                                <p className="text-slate-300 text-xl font-medium leading-relaxed">
                                    A teoria gratuita acaba aqui. O Protocolo Nexus Black não é um curso, é um arsenal nuclear de vendas.
                                </p>
                            </div>
                            <a 
                                href="https://nexusdigital.net.br" 
                            ...
                            >
                                ACESSAR PROTOCOLO <ArrowUpRight size={24} strokeWidth={3} />
                            </a>
                        </div>
                    </div>
               )}
               ...
               <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                   <GlassCard className="flex items-center gap-8 p-8 border-t-4 border-t-blue-500">
                       <div className="p-5 bg-blue-500/10 rounded-2xl text-blue-400"><Target size={40} /></div>
                       <div>
                   ...
                   <GlassCard className="flex items-center gap-8 p-8 border-t-4 border-t-green-500">
                   ...
                   <GlassCard className={`flex items-center gap-8 p-8 border-t-4 transition-all duration-500 ${isMasterComplete ? 'border-t-amber-500 bg-amber-500/5 cursor-pointer hover:scale-[1.02]' : 'border-t-slate-700 opacity-70 grayscale'}`}>
          ...
          <div className="space-y-12 animate-fadeIn">
            ...
            <div className="grid gap-12">
                 ...
                 {level === 'iniciante' && (
                      <>
                          <div className="space-y-6">
                              <h3 className="text-2xl font-black text-cyan-400 uppercase tracking-widest pl-6 border-l-8 border-cyan-500">Semana 1: Fundação & Identidade</h3>
                              <GlassCard className="p-0 overflow-hidden shadow-2xl">
                                  {MISSIONS_DATA.iniciante.fase1.map(t => <TaskItem key={t.id} {...t} done={completedTasks.includes(t.id)} onToggle={onTaskToggle} />)}
                                  <PhaseSummary data={MISSIONS_DATA.iniciante.fase1_summary} setActiveTab={setActiveTab} />
                              </GlassCard>
                          </div>

                          <div className="space-y-6">
                              <h3 className="text-2xl font-black text-cyan-400 uppercase tracking-widest pl-6 border-l-8 border-cyan-500">Semana 2: Estrutura de Vendas</h3>
                              <GlassCard className="p-0 overflow-hidden shadow-2xl">
                                  {MISSIONS_DATA.iniciante.fase2.map(t => <TaskItem key={t.id} {...t} done={completedTasks.includes(t.id)} onToggle={onTaskToggle} />)}
                                  <PhaseSummary data={MISSIONS_DATA.iniciante.fase2_summary} setActiveTab={setActiveTab} />
                              </GlassCard>
                          </div>

                          <div className="space-y-6">
                              <h3 className="text-2xl font-black text-cyan-400 uppercase tracking-widest pl-6 border-l-8 border-cyan-500">Semana 3: Validação & Dinheiro</h3>
                              <GlassCard className="p-0 overflow-hidden shadow-2xl">
                                  {MISSIONS_DATA.iniciante.fase3.map(t => <TaskItem key={t.id} {...t} done={completedTasks.includes(t.id)} onToggle={onTaskToggle} />)}
                                  <PhaseSummary data={MISSIONS_DATA.iniciante.fase3_summary} setActiveTab={setActiveTab} />
                              </GlassCard>
                          </div>
                 ...
                 {level === 'mestre' && (
                      <>
                          <div className="space-y-6">
                              <h3 className="text-2xl font-black text-purple-400 uppercase tracking-widest pl-6 border-l-8 border-purple-500">Semana 4: Tráfego Pago Profissional</h3>
                              <GlassCard className="p-0 overflow-hidden border-purple-500/30 shadow-2xl shadow-purple-900/10">
                                  {MISSIONS_DATA.mestre.fase1.map(t => <TaskItem key={t.id} {...t} done={completedTasks.includes(t.id)} onToggle={onTaskToggle} />)}
                                  <PhaseSummary data={MISSIONS_DATA.mestre.fase1_summary} setActiveTab={setActiveTab} />
                              </GlassCard>
                          </div>
                          
                          <div className="space-y-6">
                              <h3 className="text-2xl font-black text-purple-400 uppercase tracking-widest pl-6 border-l-8 border-purple-500">Semana 5: Otimização de LTV</h3>
                              <GlassCard className="p-0 overflow-hidden border-purple-500/30 shadow-2xl shadow-purple-900/10">
                                  {MISSIONS_DATA.mestre.fase2.map(t => <TaskItem key={t.id} {...t} done={completedTasks.includes(t.id)} onToggle={onTaskToggle} />)}
                                  <PhaseSummary data={MISSIONS_DATA.mestre.fase2_summary} setActiveTab={setActiveTab} />
                              </GlassCard>
                          </div>

                          <div className="space-y-6">
                              <h3 className="text-2xl font-black text-purple-400 uppercase tracking-widest pl-6 border-l-8 border-purple-500">Semana 6: Escala & Liberdade</h3>
                              <GlassCard className="p-0 overflow-hidden border-purple-500/30 shadow-2xl shadow-purple-900/10">
                                  {MISSIONS_DATA.mestre.fase3.map(t => <TaskItem key={t.id} {...t} done={completedTasks.includes(t.id)} onToggle={onTaskToggle} />)}
                                  <PhaseSummary data={MISSIONS_DATA.mestre.fase3_summary} setActiveTab={setActiveTab} />
                              </GlassCard>
                          </div>
                      </>
                 )}
            ...
            {TOOLKIT_DATA.map(cat => (
              <div key={cat.id}>
                <div className="flex items-center gap-5 mb-8">
                  <div className="p-4 bg-slate-800 rounded-2xl border border-white/5 shadow-lg"><cat.icon size={32} className="text-cyan-400" /></div>
                  <div>
